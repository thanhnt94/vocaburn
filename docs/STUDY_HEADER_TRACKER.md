...
> **File:** `client/src/components/StudyHeaderTracker.tsx`  
> **Alias:** `RoadmapHeaderTracker.tsx` (backward compatible)  
> **Used in:** `FlashcardPlay.tsx`, `PracticePlay.tsx`
...
- **Deck Emoji & Title**: Icon và tên bộ từ vựng (tự động truncate gọn gàng trên mobile).
- **Step Dots Stepper**: Các chấm thể hiện số chặng của Lộ trình (ví dụ: `Chặng 1/3` ●●○).
- **Mode Badge**: Huy hiệu viết tắt của chế độ học hiện tại (ví dụ: `RM`, `NEW`, `FSRS`, `REV`...).
...
| Mode ID | Emoji | Acronym (Badge) | English Label | Màu sắc / Style |
| :--- | :---: | :---: | :--- | :--- |
...
<StudyHeaderTracker
  ...
  progressPillText="8 / 20"              // (Optional) Text tùy biến, ví dụ: "Còn 3" / "3 left"
  
  // Chế độ học
  activeMode="roadmap"                   // 'roadmap' | 'new' | 'fsrs' | 'review' | 'flip' | 'mcq' | 'typing' | 'listening'
  modeBadge={{                           // (Optional) Tùy biến đè Badge
    emoji: '🛣️',
  ...
  totalCards={50}                        // Tổng số câu trong bộ
  cardsRemaining={40}                    // Số thẻ còn lại trong session
  
  // Navigation
  onExit={() => navigate('/')}           // Callback khi bấm nút [✕]
/>
```
