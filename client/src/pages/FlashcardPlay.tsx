...
export default function FlashcardPlay() {
  ...
  {/* Compact Floating Micro-Toolbar Pill (Thumb-Reachable, Centered Capsule) */}
  const renderFloatingMicroToolbar = () => {
    if (!currentQuestion) return null;

    return (
      <div className="w-full flex items-center justify-center pt-1.5 pb-0.5 px-3 z-20 pointer-events-auto">
        <div 
          data-no-flip
          className="inline-flex items-center gap-1 sm:gap-1.5 px-2.5 py-1 bg-white/95 backdrop-blur-md rounded-full border border-slate-200/90 shadow-md shadow-slate-200/50 transition-all hover:shadow-lg hover:border-slate-300"
        >
          ...
          {(() => {
            ...
            return (
              <button
                ...
                className="w-7.5 h-7.5 rounded-full bg-slate-50 hover:bg-indigo-50 text-slate-600 hover:text-indigo-600 border border-slate-200/60 flex items-center justify-center transition-all active:scale-90 cursor-pointer shadow-2xs"
          ...
          {!isFlipped ? (
            currentQuestion?.hint && (
              <button
                ...
                className={cn(
                  "w-7.5 h-7.5 rounded-full border flex items-center justify-center transition-all active:scale-90 cursor-pointer shadow-2xs",
                  showingHint 
                    ? "bg-amber-100 border-amber-300 text-amber-600 ring-2 ring-amber-400/20" 
                    : "bg-slate-50 hover:bg-amber-50 text-slate-500 hover:text-amber-600 border-slate-200/60"
                )}
                title="AI Hint"
              >
            ...
            (mainTab === 'practice' || isFlipped || showFeedback) && (
              <button
                ...
                className={cn(
                  "w-7.5 h-7.5 rounded-full border flex items-center justify-center transition-all active:scale-90 cursor-pointer shadow-2xs relative",
                  justAnswered
          ...
          {/* 3. Flip Back Button (Back face only) */}
          {isFlipped && mainTab !== 'practice' && (
            <button
              type="button"
              onClick={(e) => {
                e.stopPropagation();
              ...
              className="w-7.5 h-7.5 rounded-full bg-slate-50 hover:bg-indigo-50 text-indigo-600 border border-indigo-200/80 flex items-center justify-center transition-all active:scale-90 cursor-pointer shadow-2xs"
          ...
          <button
            ...
            className={cn(
              ...
              currentQuestion?.is_starred
                ...
                : "bg-slate-50 hover:bg-amber-50 border-slate-200/60 text-slate-400 hover:text-amber-500"
            )}
          ...
          <button
            ...
            className="w-7.5 h-7.5 rounded-full bg-slate-50 hover:bg-slate-100 text-slate-500 hover:text-slate-800 border border-slate-200/60 flex items-center justify-center transition-all active:scale-90 cursor-pointer shadow-2xs"
        ...
        <div className="max-w-2xl mx-auto w-full flex flex-col">
          {(activeBottomTab === 'flashcard' || !isFeedbackOpen) && (
            <>
              ...
              {/* FLOATING MICRO-TOOLBAR PILL (Thumb-Reachable Centered Capsule) */}
              {renderFloatingMicroToolbar()}

              {/* PRIMARY ACTION ZONE (Single Thumb-Reachable 1-Row Action Bar) */}
              <div className="w-full px-3 sm:px-4 pb-2 pt-1">
                {mainTab === 'practice' ? (
