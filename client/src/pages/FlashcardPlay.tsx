...
export default function FlashcardPlay() {
      ...
      <div className="flex-1 bg-white md:rounded-[2rem] rounded-[1.25rem] border border-slate-100 md:p-8 p-6 flex flex-col justify-between shadow-2xl shadow-indigo-100/40 min-h-0 overflow-y-auto">
        <div className="flex items-center justify-between mb-4 gap-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black tracking-wider text-slate-500 bg-slate-50 px-2.5 py-1.5 rounded-lg border border-slate-100 shadow-sm hidden sm:inline-block">
              Practice: #{practiceTotalAnswered + 1}
            </span>
          </div>
          {renderCardToolbar(true)}
        </div>
  ...
  {/* Compact Card Micro-Toolbar (Unified utility actions right on the card header) */}
  const renderCardToolbar = (isBack: boolean) => {
    if (!currentQuestion) return null;

    return (
      <div 
        data-no-flip
        className="flex items-center gap-1 sm:gap-1.5 p-1 bg-slate-100/80 hover:bg-slate-100 rounded-2xl border border-slate-200/70 shadow-2xs backdrop-blur-xs transition-colors shrink-0"
      >
            ...
            <button
              ...
              onClick={async (e) => {
                ...
                if (mainTab === 'practice') {
                  ...
                  await playCardAudio(isBack ? 'back' : 'front');
                }
              }}
              className="w-7.5 h-7.5 sm:w-8 sm:h-8 rounded-xl bg-white hover:bg-indigo-50 text-slate-500 hover:text-indigo-600 border border-slate-200/70 flex items-center justify-center transition-all active:scale-90 cursor-pointer shadow-2xs"
        ...
        {/* 2. AI Hint (Front) or Explanation Drawer (Back) */}
        {!isBack ? (
          currentQuestion?.hint && (
            <button
              ...
              className={cn(
                "w-7.5 h-7.5 sm:w-8 sm:h-8 rounded-xl border flex items-center justify-center transition-all active:scale-90 cursor-pointer shadow-2xs",
                showingHint 
                  ? "bg-amber-100 border-amber-300 text-amber-600 ring-2 ring-amber-400/20" 
                  : "bg-white hover:bg-amber-50 text-slate-500 hover:text-amber-600 border-slate-200/70"
              )}
              title="AI Hint"
            >
          ...
          (mainTab === 'practice' || isFlipped || showFeedback) && (
            <button
              ...
              className={cn(
                "w-7.5 h-7.5 sm:w-8 sm:h-8 rounded-xl border flex items-center justify-center transition-all active:scale-90 cursor-pointer shadow-2xs relative",
                justAnswered
            ...
            >
              <Lightbulb className={cn("w-3.5 h-3.5", justAnswered ? "text-white" : "text-amber-500")} />
              {justAnswered && <span className="w-1.5 h-1.5 rounded-full bg-rose-400 animate-ping absolute -top-0.5 -right-0.5" />}
            </button>
          )
        )}

        {/* 3. Flip Back Button (Back side only) */}
        {isBack && mainTab !== 'practice' && (
          <button
            type="button"
            onClick={(e) => {
              e.stopPropagation();
            ...
            className="w-7.5 h-7.5 sm:w-8 sm:h-8 rounded-xl bg-white hover:bg-indigo-50 text-indigo-600 border border-indigo-200/80 flex items-center justify-center transition-all active:scale-90 cursor-pointer shadow-2xs"
        ...
        <button
          ...
          className={cn(
            "w-7.5 h-7.5 sm:w-8 sm:h-8 rounded-xl border flex items-center justify-center transition-all active:scale-90 cursor-pointer shadow-2xs relative",
            isSelectMode
              ? "bg-amber-500 border-amber-600 text-white ring-2 ring-amber-300 drop-shadow-[0_0_8px_rgba(245,158,11,0.4)]"
              : "bg-white hover:bg-slate-50 border-slate-200/70 text-slate-500 hover:text-slate-800"
          )}
          title={isSelectMode ? "Select Mode: ON (Tap / Swipe paused to copy text)" : "Select Mode: OFF (Click to enable text selection)"}
        >
          <MousePointer className={cn("w-3.5 h-3.5", isSelectMode ? "text-white" : "text-slate-500")} />
          {isSelectMode && <span className="w-1.5 h-1.5 rounded-full bg-emerald-300 animate-ping absolute -top-0.5 -right-0.5" />}
        </button>

        {/* 5. Star / Bookmark Button */}
        <button
          type="button"
          onClick={(e) => {
            e.stopPropagation();
          ...
          className={cn(
            ...
            currentQuestion.is_starred
              ...
              : "bg-white hover:bg-amber-50 border-slate-200/70 text-slate-400 hover:text-amber-500"
          )}
          title={currentQuestion.is_starred ? "Unstar Card" : "Star Card"}
        >
          <Star className={cn("w-3.5 h-3.5", currentQuestion.is_starred && "fill-amber-500 text-amber-500")} />
        </button>
        ...
        <button
          ...
          className="w-7.5 h-7.5 sm:w-8 sm:h-8 rounded-xl bg-white hover:bg-slate-100 text-slate-500 hover:text-slate-800 border border-slate-200/70 flex items-center justify-center transition-all active:scale-90 cursor-pointer shadow-2xs"
              ...
              {mainTab === 'practice' && practiceDisabled ? (
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between gap-2 select-none">
                      <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                        <span className="text-[10px] font-black tracking-widest text-indigo-600 bg-indigo-50 px-2.5 sm:px-3 py-1.5 rounded-xl border border-indigo-100 uppercase shadow-sm">
                          FRONT
                        </span>
                        <span className="text-[10px] font-black tracking-wider text-white bg-indigo-500 px-2.5 sm:px-3 py-1.5 rounded-xl border border-indigo-600 shadow-sm">
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                        <span className="text-[10px] font-black tracking-widest text-emerald-600 bg-emerald-50 px-2.5 sm:px-3 py-1.5 rounded-xl border border-emerald-100 uppercase shadow-sm">
                          BACK
                        </span>
                        <span className="text-[10px] font-black tracking-wider text-white bg-emerald-500 px-2.5 sm:px-3 py-1.5 rounded-xl border border-emerald-600 shadow-sm">
                          {currentQuestion?.original_index ?? (currentIndex + 1)}
                        </span>
                        <div className="hidden sm:block">
                          {currentQuestion && getMasteryPill(currentQuestion)}
        ...
        <div className="max-w-2xl mx-auto w-full flex flex-col">
          {(activeBottomTab === 'flashcard' || !isFeedbackOpen) && (
            <>
              ...
              {/* PRIMARY ACTION ZONE (Single Thumb-Reachable 1-Row Action Bar) */}
