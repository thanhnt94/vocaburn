  ...
  return (
    <>
      ...
      <div className="md:hidden fixed inset-0 top-0 bottom-[60px] flex flex-col bg-slate-50 overflow-hidden z-[90]">
        {/* Top Header */}
        <div className="bg-white px-4 pt-3 pb-3 border-b border-slate-100 flex items-center justify-between shrink-0 shadow-2xs">
          ...
          <Link
            to="/library"
            className="w-8 h-8 rounded-xl bg-indigo-50 border border-indigo-100 flex items-center justify-center text-indigo-600 font-bold active:scale-95 transition-all"
        ...
        <div className="px-4 py-2.5 bg-white border-b border-slate-100 shrink-0">
          <div className="grid grid-cols-3 gap-2 text-center">
            <div className="p-2 bg-indigo-50/60 rounded-xl border border-indigo-100/60">
              <span className="text-[8px] font-black text-indigo-500 uppercase tracking-widest block mb-0.5">Tiến độ</span>
              <span className="text-base font-black text-indigo-950">{completedTodayCount}/{decks.length}</span>
              <span className="text-[8px] font-bold text-indigo-600 block">hôm nay</span>
            </div>

            <div className="p-2 bg-emerald-50/60 rounded-xl border border-emerald-100/60">
              <span className="text-[8px] font-black text-emerald-500 uppercase tracking-widest block mb-0.5">Ghi nhớ</span>
              <span className="text-base font-black text-emerald-950">
                {decks.length > 0
                  ? `${Math.round(decks.reduce((acc, d) => acc + (d.status?.retention_rate || 0), 0) / decks.length)}%`
                  : '—'}
              </span>
              <span className="text-[8px] font-bold text-emerald-600 block">retention</span>
            </div>

            <div className="p-2 bg-orange-50/60 rounded-xl border border-orange-100/60">
              <span className="text-[8px] font-black text-orange-500 uppercase tracking-widest block mb-0.5">Streak</span>
              <span className="text-base font-black text-orange-950">
                🔥 {Math.max(0, ...decks.map(d => d.status?.streak || 0))}d
              </span>
              <span className="text-[8px] font-bold text-orange-600 block">cao nhất</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 scrollbar-thin">
          <div className="flex items-center justify-between px-0.5">
            ...
            <Link to="/library" className="text-[10px] font-bold text-indigo-600">
              + Thêm bộ thẻ
            </Link>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="bg-white rounded-2xl p-6 text-center border border-slate-200/80 shadow-2xs">
              ...
              <Link
                to="/library"
                className="px-4 py-2.5 bg-indigo-600 text-white font-black text-xs uppercase tracking-wider rounded-xl shadow-xs inline-block"
              >
                Vào Thư Viện 📚
              </Link>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-start gap-3">
                      <div className="w-12 h-12 rounded-xl border bg-gradient-to-br from-indigo-50 to-purple-50 border-slate-200 flex items-center justify-center shrink-0 overflow-hidden">
                        {item.cover_image ? (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-1 mb-1">
                          ...
                          {isAllDone ? (
                            <span className="px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-600 text-[8px] font-black uppercase tracking-wider shrink-0 border border-emerald-100">
                              ✓ Xong
                            </span>
                          ) : (
                            <span className="px-2 py-0.5 rounded-full bg-rose-50 text-rose-600 text-[8px] font-black uppercase tracking-wider shrink-0 border border-rose-100">
                        ...
                        <div className="flex items-center gap-1 mt-1 flex-wrap">
                          {pipeline.map((st: any, idx: number) => (
                            <span
                              key={idx}
                              className={cn(
                                "px-1.5 py-0.5 rounded-md text-[8px] font-black flex items-center gap-0.5 border",
                                st.done ? "bg-emerald-50 text-emerald-700 border-emerald-200" :
                                idx === s.current_step_index ? "bg-indigo-50 text-indigo-700 border-indigo-200" :
                                "bg-slate-50 text-slate-400 border-slate-200"
                              )}
                            >
                              {st.done ? '✓' : `${idx + 1}.`} {st.label}
                            </span>
                          ))}
                        </div>
                      </div>
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-slate-100 text-[9px] font-bold text-slate-500">
                      <span>🔥 <strong className="text-orange-600">{s.streak || 0}d</strong></span>
                      <span>🧠 <strong className="text-indigo-600">{s.retention_rate || 0}%</strong></span>
                      <span>📚 Còn <strong className="text-slate-700">{s.unlearned_cards || 0}</strong> thẻ</span>
                      
                      <Link
                        to={`/flashcard/${item.deck_id}/roadmap`}
                        className="px-2.5 py-1 bg-slate-100 text-slate-700 rounded-lg text-[9px] font-black uppercase tracking-wider active:scale-95"
                      >
                        Lộ Trình 🗺️
                      </Link>
                    </div>
