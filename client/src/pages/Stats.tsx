    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      ...
      >
        ...
        <div className="bg-white border-b border-slate-200/80 shrink-0 z-30 shadow-xs">
          ...
          {/* Sub-Tabs Row */}
          <div className="px-3.5 pb-2.5 overflow-x-auto no-scrollbar">
            {activeTab === 'personal' ? (
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin">
          <AnimatePresence mode="wait">
            {activeTab === 'personal' && (
              <motion.div
                key={`personal-${mobilePersonalSubTab}`}
              ...
              >
                {/* 📍 SUB-TAB 0: TỔNG QUAN */}
                {mobilePersonalSubTab === 0 && (
                  <>
                    {/* Period Selector Bar */}
                    <div className="flex bg-white p-1 rounded-2xl border border-slate-200/80 shadow-2xs overflow-x-auto no-scrollbar">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <MetricCard label="Accurate Rate" value={`${periodSummary.global_accuracy}%`} sub="Tỷ lệ chính xác" icon={TargetIcon} color="text-indigo-600" bg="bg-indigo-50" />
                      <MetricCard label="Study Duration" value={`${periodSummary.total_time_hours}h`} sub="Tổng thời gian" icon={Clock} color="text-emerald-600" bg="bg-emerald-50" />
                      <MetricCard label="Attempted Cards" value={periodSummary.total_questions.toLocaleString()} sub="Số thẻ đã ôn" icon={Layers} color="text-amber-600" bg="bg-amber-50" />
                      <MetricCard label="Correct Score" value={periodSummary.total_correct.toLocaleString()} sub="Số câu đúng" icon={Zap} color="text-rose-600" bg="bg-rose-50" />
                    </div>
                    ...
                    <div className="bg-white rounded-3xl p-4 border border-slate-200/90 shadow-sm space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-[10px]">
                        <div className="p-3 bg-slate-50 rounded-2xl border border-slate-100">
                          <span className="text-[8px] font-black text-slate-400 uppercase block">Khối lượng</span>
                          <span className="text-base font-black text-slate-900 mt-0.5 block">{periodSummary.total_questions.toLocaleString()} thẻ</span>
                        </div>
                        <div className="p-3 bg-slate-50 rounded-2xl border border-slate-100">
                          <span className="text-[8px] font-black text-slate-400 uppercase block">Chính xác</span>
                          <span className="text-base font-black text-indigo-600 mt-0.5 block">{periodSummary.global_accuracy}%</span>
                        </div>
                        <div className="p-3 bg-slate-50 rounded-2xl border border-slate-100">
                          <span className="text-[8px] font-black text-slate-400 uppercase block">Tập trung</span>
                          <span className="text-base font-black text-emerald-600 mt-0.5 block">{periodSummary.total_time_minutes} phút</span>
                        </div>
                        <div className="p-3 bg-slate-50 rounded-2xl border border-slate-100">
                          <span className="text-[8px] font-black text-slate-400 uppercase block">Ngày đỉnh cao</span>
                          <span className="text-base font-black text-amber-600 mt-0.5 block truncate">{periodSummary.best_day}</span>
                        </div>
                      </div>
                    </div>
                  </>
                ...
                {mobilePersonalSubTab === 1 && (
                  <>
                    ...
                    {/* Charts Carousel */}
                    <div className="bg-white rounded-3xl border border-slate-200/90 p-4 shadow-sm space-y-4">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-black text-slate-900 uppercase">{charts[activeChart].title}</span>
                          ...
                          {charts.map((_, idx) => (
                            <button
                              ...
                              className={cn(
                                ...
                                activeChart === idx ? "w-6 bg-indigo-600" : "w-2 bg-slate-200"
                              )}
                            />
                          ))}
                        </div>
            ...
            {activeTab === 'community' && (
              <motion.div
                key={`community-${mobileCommunitySubTab}`}
                initial={{ opacity: 0, x: 20 }}
                animate={{ opacity: 1, x: 0 }}
                exit={{ opacity: 0, x: -20 }}
                transition={{ duration: 0.2 }}
                ...
                {mobileCommunitySubTab === 0 && (
                  <div className="bg-white rounded-3xl p-4 border border-slate-200/90 shadow-sm space-y-3">
                        ...
                        {(['xp', 'streak', 'questions', 'accuracy'] as const).map(tab => (
                          <button
                            ...
                            className={cn(
                              ...
                              activeLeaderboardTab === tab ? "bg-indigo-600 text-white" : "bg-slate-100 text-slate-500"
                            )}
                          >
                            {tab}
                          </button>
                    ...
                    {currentLeaderboard.user_rank !== -1 && (
                      <div className="p-3 bg-indigo-600 rounded-2xl text-white flex items-center justify-between text-xs">
                        <div>
                          <span className="text-[8px] font-black uppercase text-indigo-200 block">Thứ hạng của bạn</span>
      ...
      {/* 💻 DESKTOP LAYOUT (>= md) */}
      <div className="hidden md:block pb-[120px] md:pb-10">
        {/* Sticky Tab Bar */}
              ...
              {activeTab === 'personal' && (
                 ...
                 >
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
                       <MetricCard label="Accurate Rate" value={`${periodSummary.global_accuracy}%`} sub="Tỷ lệ chính xác" icon={TargetIcon} color="text-indigo-600" bg="bg-indigo-50" />
                       <MetricCard label="Study Duration" value={`${periodSummary.total_time_hours}h`} sub="Tổng thời gian học" icon={Clock} color="text-emerald-600" bg="bg-emerald-50" />
                       <MetricCard label="Attempted Cards" value={periodSummary.total_questions.toLocaleString()} sub="Số câu/thẻ đã ôn" icon={Layers} color="text-amber-600" bg="bg-amber-50" />
                       <MetricCard label="Correct Score" value={periodSummary.total_correct.toLocaleString()} sub="Số câu trả lời đúng" icon={Zap} color="text-rose-600" bg="bg-rose-50" />
                    </div>
                      ...
                      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                        <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                          ...
                          <div className="flex items-baseline gap-1.5 mt-1">
                            ...
                            <span className="text-[8px] font-black px-1.5 py-0.5 rounded-full bg-emerald-50 text-emerald-600">Thẻ</span>
                          </div>
                        </div>
                        ...
                        <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                          ...
                          <div className="flex items-baseline gap-1.5 mt-1">
                            ...
                            <span className="text-[8px] font-black px-1.5 py-0.5 rounded-full bg-indigo-50 text-indigo-600">Chính xác</span>
                          </div>
                        </div>

                        <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                          <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block">Thời gian tập trung</span>
                          <div className="text-2xl font-black text-emerald-600 mt-1">{periodSummary.total_time_minutes} phút</div>
                        </div>

                        <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                          <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block">Ngày đạt đỉnh</span>
                          <div className="text-2xl font-black text-indigo-600 mt-1 truncate">{periodSummary.best_day}</div>
                        </div>
                      </div>
                    </div>
                                 ...
                                 {charts.map((c, idx) => (
                                   <button 
                                      ...
                                      className={cn(
                                        ...
                                        activeChart === idx ? "bg-slate-900 text-white border-slate-900" : "bg-white text-slate-400 border-slate-100 hover:border-slate-200"
                                      )}
                                   >
                                      {c.title}
                                   </button>
                                       ...
                                       <ResponsiveContainer width="100%" height="100%">
                                          {charts[activeChart].type === 'area' ? (
                                             <AreaChart data={charts[activeChart].data as any[]}>
                                                ...
                                                />
                                                <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 8, fontWeight: 900, fill: '#94a3b8' }} />
                                                <Tooltip contentStyle={{ borderRadius: '1rem', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)', fontSize: '10px', fontWeight: 900 }} />
                                             ...
                                             <BarChart data={charts[activeChart].data as any[]}>
                                                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
                                                <XAxis dataKey="hour" axisLine={false} tickLine={false} tick={{ fontSize: 8, fontWeight: 900, fill: '#94a3b8' }} interval={2} />
                                                <Tooltip 
                                                  cursor={{fill: '#f8fafc'}}
                                                  content={({ active, payload }: any) => {
                                                    if (active && payload && payload.length) {
                                ...
                                <div className="h-[200px] w-full mt-4 -ml-4">
                                  <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={leitnerStats.box_distribution}>
                                      ...
                                      <XAxis dataKey="label" axisLine={false} tickLine={false} tick={{ fontSize: 8, fontWeight: 900, fill: '#94a3b8' }} />
                                      <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 8, fontWeight: 900, fill: '#94a3b8' }} />
                                      <Tooltip cursor={{fill: '#f8fafc'}} contentStyle={{ borderRadius: '1rem', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)', fontSize: '10px', fontWeight: 900 }} />
                                ...
                                <div className="space-y-4">
                                  <div className="p-4 bg-white border border-slate-100 rounded-2xl">
                                    ...
                                    <div className="flex items-baseline gap-2 mt-1">
                                      <span className="text-2xl font-black text-emerald-600 leading-none">{speedAccuracyStats.avg_speed_correct}s</span>
                                    </div>
                                  ...
                                  <div className="p-4 bg-white border border-slate-100 rounded-2xl">
                                    ...
                                    <div className="flex items-baseline gap-2 mt-1">
                                      <span className="text-2xl font-black text-rose-600 leading-none">{speedAccuracyStats.avg_speed_wrong}s</span>
                                    </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                              <div className="p-6 bg-white border border-slate-100 rounded-[2rem] flex flex-col justify-between hover:border-violet-200 transition-all">
                                <div>
                                  ...
                                  <div className="flex items-baseline gap-2">
                                    <span className="text-3xl font-black text-slate-900">{practiceStats.mcq.correct + practiceStats.mcq.wrong}</span>
                              ...
                              <div className="p-6 bg-white border border-slate-100 rounded-[2rem] flex flex-col justify-between hover:border-violet-200 transition-all">
                                <div>
                                  ...
                                  <div className="flex items-baseline gap-2">
                                    <span className="text-3xl font-black text-slate-900">{practiceStats.typing.correct + practiceStats.typing.wrong}</span>
                                    <span className="text-[10px] font-black text-slate-400 uppercase">lượt làm</span>
                                  </div>
                                  <div className="mt-4 space-y-1.5">
                                    <div className="flex justify-between text-[10px] font-semibold text-slate-500">
                              ...
                              <div className="p-6 bg-white border border-slate-100 rounded-[2rem] flex flex-col justify-between hover:border-violet-200 transition-all">
                                <div>
                                  ...
                                  <div className="flex items-baseline gap-2">
                                    <span className="text-3xl font-black text-slate-900">{practiceStats.listening.correct + practiceStats.listening.wrong}</span>
                                    <span className="text-[10px] font-black text-slate-400 uppercase">lượt làm</span>
                                  </div>
                                  <div className="mt-4 space-y-1.5">
                                    <div className="flex justify-between text-[10px] font-semibold text-slate-500">
                               ...
                               {personal.category_performance.slice(0, 6).map((cat, idx) => (
                                 <div key={idx}>
                                     ...
                                     <div className="h-1.5 w-full bg-white rounded-full overflow-hidden border border-slate-100">
                                        <motion.div initial={{ width: 0 }} animate={{ width: `${cat.accuracy}%` }} className="h-full bg-indigo-600 rounded-full" />
                                     </div>
                                 </div>
                               ))}
                            </div>
                          ...
                          <>
                             ...
                             <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1 no-scrollbar">
                                {remainingUsers.length === 0 && topThree.length === 0 ? (
                                   <div className="py-10 text-center text-slate-300 font-bold text-xs">Chưa có dữ liệu xếp hạng nào.</div>
                                ) : remainingUsers.length === 0 ? (
                                   <div className="py-4 text-center text-[9px] font-black text-slate-300 uppercase tracking-widest">Đã hiển thị hết danh sách</div>
                                ) : (
                                   remainingUsers.map((user: any) => {
                                      ...
                                      return (
                                         <div key={user.user_id} className="flex items-center gap-3 p-3 rounded-2xl bg-slate-50 border border-slate-100 hover:border-indigo-100 transition-all hover:scale-[1.005]">
                                            <div className="w-6 text-[10px] font-black text-slate-400 text-center">#{user.rank}</div>
                                            <div className="w-8 h-8 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-xs font-black text-slate-700 shrink-0">{initial}</div>
                                            <div className="flex-1 min-w-0">
                                ...
                                <div className="mt-4 p-4 bg-indigo-600 rounded-3xl border border-indigo-500 shadow-lg shadow-indigo-600/20 text-white flex items-center justify-between gap-4">
                                   <div className="flex items-center gap-3 min-w-0">
                                      ...
                                      <div className="min-w-0">
                                         ...
                                         <h4 className="text-[11px] font-black truncate uppercase leading-tight">Bạn đang đứng thứ <span className="text-amber-300">#{currentLeaderboard.user_rank}</span></h4>
                                      </div>
                                   </div>
                                   <div className="text-right shrink-0">
