...
export default function LeaderboardTab({
  ...
  return (
    <div className="space-y-3 text-left max-w-5xl mx-auto flex flex-col min-h-0">
      {/* 🌟 My Rank Banner - Compact Sleek Ribbon */}
      {currentLeaderboard.user_rank !== -1 && (
        <motion.div
          initial={{ opacity: 0, y: 8 }}
          animate={{ opacity: 1, y: 0 }}
          className="bg-slate-900 rounded-2xl px-3.5 py-2.5 text-white border border-slate-800 shadow-md flex items-center justify-between gap-3 relative overflow-hidden shrink-0"
        >
      ...
      <div className="bg-white rounded-3xl border border-slate-200/80 p-3.5 sm:p-5 shadow-sm relative overflow-hidden flex flex-col min-h-0">
        {isLoading ? (
          ...
          <>
            {/* Top 3 Podium (Đỉnh vinh danh nổi bật) */}
            {topThree.length > 0 && (
              <div className="flex items-end justify-center gap-2.5 sm:gap-6 py-4 sm:py-5 border-b border-slate-100 bg-gradient-to-b from-amber-50/30 via-slate-50/40 to-transparent rounded-2xl mb-3 px-2 shrink-0">
                {(() => {
                  const podiums = [
                    { item: topThree[1], pos: 2, height: 'h-16 sm:h-18', color: 'from-slate-100 to-slate-200 border-slate-300', text: 'text-slate-700', ring: 'border-slate-300', badgeBg: 'bg-slate-500' },
                    { item: topThree[0], pos: 1, height: 'h-22 sm:h-26', color: 'from-amber-100 via-amber-200 to-amber-300 border-amber-400', text: 'text-amber-800', ring: 'border-amber-400 ring-4 ring-amber-200', badgeBg: 'bg-amber-500' },
                    { item: topThree[2], pos: 3, height: 'h-13 sm:h-15', color: 'from-orange-100 to-orange-200 border-orange-300', text: 'text-orange-800', ring: 'border-orange-300', badgeBg: 'bg-orange-500' }
                  ].filter(p => p.item)
                      ...
                      <div key={user.user_id} className="flex flex-col items-center w-22 sm:w-28 shrink-0 text-center">
                        <div className="relative mb-2">
                          ...
                          <div className={cn(
                            ...
                            pod.pos === 1 ? "w-12 h-12 sm:w-14 sm:h-14 text-sm sm:text-base" : "w-10 h-10 sm:w-11 sm:h-11 text-xs sm:text-sm",
                            pod.ring
                          )}>
                            {initial}
                            <div className={cn(
                              "absolute -bottom-1 -right-1 w-4.5 h-4.5 sm:w-5 sm:h-5 rounded-full border-2 border-white flex items-center justify-center text-[8.5px] sm:text-[9.5px] font-black text-white shadow-xs",
                              pod.badgeBg
                            )}>
                              {pod.pos}
                            </div>
                        ...
                        <div className={cn(
                          "w-full mt-2.5 rounded-t-xl flex flex-col justify-end items-center pb-2 bg-gradient-to-t shadow-2xs border-t border-x",
                          pod.height, pod.color
                        )}>
                          <span className={cn("text-[10px] sm:text-xs font-black tracking-tight leading-none mb-0.5", pod.text)}>
            ...
            {/* List Top 4 - 50 (Cuộn nội bộ khi màn hình ngắn) */}
            <div className="space-y-1.5 flex-1 min-h-0 max-h-[340px] sm:max-h-[420px] overflow-y-auto custom-scrollbar pr-1">
