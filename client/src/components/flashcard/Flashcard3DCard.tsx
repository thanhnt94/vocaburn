...
export const Flashcard3DCard: React.FC<Flashcard3DCardProps> = ({
  ...
  dragOffset,
  canDragRate,
  hasBackOverflow: _hasBackOverflow,
  backScrollRef,
  handleCardDrag,
  handleCardDragEnd,
  cardDragControls,
  ...
  const renderCardTopHeader = (face: 'front' | 'back') => {
    ...
    return (
      <div 
        className="flex items-center justify-between select-none w-full gap-2"
        style={isFront ? undefined : { touchAction: 'none' }}
      >
        {/* Left Cluster: Unified Face/Index Badge + Review Count Pill */}
        <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
          {/* Unified Face & Index Pill */}
          <div className={cn(
            "inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xl border font-black text-[10px] shadow-2xs transition-all",
            isFront 
              ? "bg-indigo-50/90 border-indigo-200/80 text-indigo-700" 
              : "bg-emerald-50/90 border-emerald-200/80 text-emerald-700"
          )}>
            <span className={cn(
              "tracking-widest uppercase text-[9px] sm:text-[9.5px]",
              isFront ? "text-indigo-600" : "text-emerald-600"
            )}>
              {isFront ? 'FRONT' : 'BACK'}
            </span>
            <span className={cn(
              "font-bold text-[8px] opacity-40",
              isFront ? "text-indigo-900" : "text-emerald-900"
            )}>•</span>
            <span className={cn(
              "font-mono tracking-tight font-black",
              isFront ? "text-indigo-800" : "text-emerald-800"
            )}>
              #{originalIndex}
            </span>
          </div>
          ...
          <button
            ...
            className={cn(
              "inline-flex items-center gap-1.5 px-2 sm:px-2.5 py-1 rounded-xl border text-[10px] font-bold transition-all active:scale-95 cursor-pointer shadow-2xs group",
              reviewCount > 0 
                ? "bg-slate-50/90 hover:bg-indigo-50/70 border-slate-200/80 hover:border-indigo-200 text-slate-600 hover:text-indigo-700" 
                : "bg-slate-50/60 hover:bg-slate-100/80 border-slate-200/60 text-slate-400 hover:text-slate-600"
            )}
            title={`Reviewed ${reviewCount} times (${correctCount} correct${accuracy !== null ? `, ${accuracy}% accuracy` : ''}). Click for card stats.`}
          >
            <Eye className="w-3.5 h-3.5 shrink-0 text-slate-400 group-hover:text-indigo-500 transition-colors" />
            <span className="font-black text-slate-700 group-hover:text-indigo-800 font-mono">
              {reviewCount}
            </span>
            <span className="hidden sm:inline text-slate-500 text-[9.5px]">
              {reviewCount === 1 ? 'view' : 'views'}
            </span>
            {accuracy !== null && (
              <>
        ...
        {/* Right Cluster: Mastery Status Pill + Star Bookmark */}
        <div className="flex items-center gap-1.5 shrink-0">
          {currentQuestion && (
            <div 
              ...
              title="Click to view card mastery details"
            >
              {getMasteryPill(currentQuestion)}
            </div>
          )}
          {currentQuestion && (
            <button
              ...
              className={cn(
                "w-7 sm:w-7.5 h-7 sm:h-7.5 flex items-center justify-center rounded-xl border transition-all active:scale-90",
                currentQuestion.is_starred
                  ? "bg-amber-50 border-amber-300 text-amber-500 shadow-xs ring-2 ring-amber-400/20"
                  : "bg-slate-50/80 border-slate-200/70 text-slate-400 hover:text-amber-500 hover:bg-amber-50/50 hover:border-amber-200"
              )}
        ...
        >
          ...
          >
            ...
            <div 
              ...
              style={{
                touchAction: isSelectMode ? 'auto' : (canDragRate ? 'none' : 'pan-y'),
